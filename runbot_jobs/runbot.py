...
class runbot_repo(models.Model):
    ...
    db_name = fields.Char(string='Database name to replicated')
...
class runbot_build(models.Model):
    ...
    def create(self, vals):
        ...
        if build_id.repo_id.db_name:
            build_id.write({'db_name': build_id.repo_id.db_name})
    ...
    def _job_25_restore(self, build, log_path):
        if not build.db_name:
            return -2
        self._local_pg_createdb("%s-custom" % build.dest)
        build._log('restore', 'Restoring %s on %s-custom' %(build.db_name, build.dest))
        cmd = "pg_dump %s | psql %s-custom" % (build.db_name, build.dest)
