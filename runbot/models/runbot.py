...
class Runbot(models.AbstractModel):
    ...
    def _start_nginx(self):
        ...
        with file_open(self.env['runbot.runbot']._path('nginx', 'main_nginx.pid'), 'w') as f:
    ...
    def _check_main_nginx(self):
        ...
        if current_main_nginx_pid is not None and known_main_nginx_pid != current_main_nginx_pid:
            _logger.info('main nginx restarted (%s -> %s), restarting runbot nginx',
                         known_main_nginx_pid, current_main_nginx_pid)
            try:
