...
class UpgradeError(models.Model):
    _name = 'runbot.upgrade.regex'
    _description = 'Upgrade error'
    ...
    def _parse_upgrade_errors(self):
        ...
        for log in ir_logs:
            for upgrade_regex in upgrade_regexes:
                m = re.match(upgrade_regex.regex, log.message)
                if m:
        ...
        if exception:
            ...
            res = {
                ...
                'context': {
                        ...
                        'default_info': 'Automaticaly generated from build %s' % self.id
                    }
                }
            return res
        else:
