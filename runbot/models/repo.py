...
class runbot_repo(models.Model):
    ...
    def _gc_testing(self, host):
        ...
        Build = self.env['runbot.build']
        domain_host = self.build_domain_host(host)
        testing_builds = Build.search(domain_host + [('local_state', '=', 'testing')])
        ...
        for build in testing_builds:
            if not build.branch_id.sticky:
                newer_candidates = Build.search([('id', '>', build.id), ('branch_id', '=', build.branch_id.id), ('build_type', '=', 'normal'), ('parent_id', '=', False), ('local_state', '=', 'testing')])
                if newer_candidates.exists():
                    btk = build if not build.parent_id else build._get_top_parent()
                    btk._log('repo._update_git', 'Build automatically killed, newer build found.', level='WARNING')
                    builds_to_kill |= btk
        btk.write({'requested_action': 'deathrow'})
