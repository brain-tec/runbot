...
class BuildError(models.Model):
    ...
    def _onchange_test_tags(self):
        for record in self:
            if record.test_tags and record.version_ids:
    ...
    def action_link_errors(self):
        ...
        self = self.sorted(lambda error: (not error.test_tags, not error.responsible, error.error_count, error.id))
        base_error = self[0]
        base_error._merge(self - base_error)
...
class BuildErrorContent(models.Model):
    ...
    def action_link_errors_contents(self):
        ...
        if len(self) < 2:
            return
        # sort self so that the first one is the one that has test tags or responsible, or the oldest.
        self = self.sorted(lambda ec: (not ec.error_id.test_tags, not ec.error_id.responsible, ec.error_id.error_count, ec.id))
        base_error = self[0].error_id
        base_error._merge(self.error_id - base_error)
