...
class Runbot(Controller):
    ...
    @route(['/runbot/quick_connect/<int:branch_id>'], type='http', auth="public", website=True)
    def fast_launch(self, branch_id, **post):
        """Connect to the running Odoo instance"""
        Build = request.env['runbot.build']
        branch = request.env['runbot.branch'].browse(branch_id)
        domain = [('branch_id.id', '=', branch_id), ('config_id', '=', branch.config_id.id)]
