...
class TestBuildError(common.TransactionCase):
    ...
    def test_build_scan(self, mock_get_params):
        ...
        # this assertion does not seem to work ???
        # self.assertIn(build_error, new_build_error.child_ids, 'The old error should be linked to the new one')
        self.assertEqual(build_error.parent_id, new_build_error, 'The old error should be linked to the new one')
