...
class TestHost(RunbotCase):
    ...
    def setUp(self):
        ...
        self.start_patcher('host_local_logs_cursor', 'odoo.addons.runbot.models.host.local_logs_cursor')

    def test_build_logs(self):

        host = self.env['runbot.host'].create({'name': 'test_host'})
        ...
        host.process_logs()
        self.patchers['host_local_logs_cursor'].assert_called()
        self.assertEqual(
        ...
        # check that a warn log sets the build in warining
